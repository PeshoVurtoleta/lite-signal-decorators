...
const SCENARIOS = [
    ...
    { name: "localto-torture", file: "localto-torture.mjs", group: "semantic", floor: "1.5.0", about: "S8-A2 zero-alloc @localTo read/write storm + S8-A6 ABA-stale interleave lattice + S8-A3 pooled park/reinit + S8-A4/A5 tracking edges + pure-compute reads" },
    { name: "introspection-torture", file: "introspection-torture.mjs", group: "semantic", floor: "1.5.0", about: "S9 T10 zero-alloc forEachReactive walk (1e6, control-relative) + T11 snapshotOf bytes/op reported + A4 construct/snapshot/dispose retention (lite-leak 0)" },
    { name: "fleet-torture", file: "fleet-torture.mjs", group: "semantic", floor: "1.5.0", about: "S11 A1 zero-alloc createFleet acquire/release storm (major 0, poolGrowths 0, control-relative) + A3 release node-delta + A4 cold exhausted/foreign throws + A2 1000 fleet lifecycles conservation (lite-leak 0)" },
